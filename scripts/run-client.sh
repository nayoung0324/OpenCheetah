...
then
  ...
  if ! contains "cheetah SCI_HE" $1; then
    echo -e "Usage: run-client.sh ${RED}[cheetah|SCI_HE]${NC} [sqnet|resnet18|resnet50|densenet121]"
	exit 1
  fi

  if ! contains "sqnet resnet18 resnet50 densenet121" $2; then
    echo -e "Usage: run-client.sh [cheetah|SCI_HE] ${RED}[sqnet|resnet18|resnet50|densenet121]${NC}"
	exit 1
  fi
  if [ "$1" = "SCI_HE" ] && [ "$2" = "resnet18" ]; then
    echo -e "${RED}resnet18 only has cheetah benchmark support.${NC}"
    exit 1
  fi
  ...
  if [ "$2" = "resnet18" ]; then
    build/bin/$2-$1 r=2 k=$FXP_SCALE ell=$SS_BITLEN nt=$NUM_THREADS ip=$SERVER_IP p=$SERVER_PORT 1>$1-$2_client.log
  else
